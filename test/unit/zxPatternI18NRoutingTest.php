...
/*
 * Extended symfony sfPatternRouting tests to test new routing features
 */

require_once(dirname(__FILE__).'/../bootstrap/unit.php');

$t = new lime_test(152, new lime_output_color());

class zxPatternI18NRoutingTest extends zxPatternI18NRouting
{
...
$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, $options);

$t->diag('sfPatternRouting part tests (for BC)');
// ->getRoutes()
$t->diag('->getRoutes()');
$r->clearRoutes();
...
$t->diag('suffix');
$r->clearRoutes();
$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, array_merge($options, array('suffix' => '.html')));
...
// separators
$t->diag('separators');
$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, array_merge($options, array('segment_separators' => array('/', ';', ':', '|', '.', '-', '+'))));
$r->clearRoutes();
...
$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, array_merge($options, array('variable_prefixes' => array(':', '$'))));
...
// cache-in
$rCached = new zxPatternI18NRoutingTest($dispatcher, $cache, array_merge($options, array('lazy_routes_deserialize' => true)));
$rCached->parse('/first');
...
$rCached = new zxPatternI18NRoutingTest(new sfEventDispatcher(), new sfNoCache(), $options);
...
$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, array('generate_shortest_url' => true, 'extra_parameters_as_query_string' => false, 'suffix' => '.html'));
...
$t->comment('-------------------------------');
$t->comment('zxPatternI18NRouting part tests');

sfConfig::set('sf_i18n', true);
$options = array('use_cultures' => array(), 'culture_into_url' => true);
$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, $options);

$t->diag('->getRoutes() (culture_into_url = true, use_cultures = array(), sf_i18n = true )');
$r->clearRoutes();
$r->connect('xz_test_route_1', new sfRoute('/test/group/view'));
$r->connect('xz_test_route_2', new sfRoute('/test/group/edit'));
$routes = $r->getRoutes();
$t->is(count($routes), 4, '->getRoutes() each route gets :sf_culture possibility');
$t->ok(isset($routes['xz_test_route_1_ntr']), '->getRoutes() test route 1 has i18n possibility');
$t->ok(isset($routes['xz_test_route_2_ntr']), '->getRoutes() test route 2 has i18n possibility');

$r->connect('xz_test_route_3', new sfRoute('/test/group/culture/:sf_culture'));
$routes = $r->getRoutes();
$t->ok(!isset($routes['xz_test_route_3_ntr']), '->getRoutes() routes with defined :sf_culture are leaved untouched');

$r = new zxPatternI18NRoutingTest(new sfEventDispatcher(), null, $options);
$t->diag('->getRoutes() (culture_into_url = true, use_cultures = array(\'gr\',\'fr\'), sf_i18n = true )');
$r->clearRoutes();
$r->connect('xz_test_route_1', new sfRoute('/test/group/view'));
$r->connect('xz_test_route_2', new sfRoute('/test/group/edit'));
$r->connect('xz_test_route_3', new sfRoute('/test/group/culture/:sf_culture'));
$routes = $r->getRoutes();
$t->is(count($routes), 5, '->getRoutes() if translation is not possible, additional routes are not created');

$t->is($r->generate('xz_test_route_1', array('sf_culture' => 'en')), '/en/test/group/view', '->generate() given culture is placed into URL (even if base routing does not have :sf_culture defnined)');
